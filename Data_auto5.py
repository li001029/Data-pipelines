...
def numerical_Label(cleandata,classtype):
    ...
    for i in range(0,len(cleandata)):
        label+=[cleandata[i][-1]]
        cleandata[i].pop()
    return cleandata,label

def construct_graph(cleandata,k):
    n=len(cleandata)
    I = np.zeros((n,k))
    J = np.zeros((n,k))
    D = np.zeros((n,k))
    ...
    for i in range(0,n):
        for j in range(0,k):
            I[i][j]=i
        D[i],J[i]=knn(cleandata,i,k)
    return I,J,D


def knn(cleandata,i,k):
    n=len(cleandata)
    dist=[]
    for j in range(0,n):
        #Gon
        distance=abs(cleandata[i][26]-cleandata[j][26])+abs(cleandata[i][2]-cleandata[j][2])
        #Epiphysis
        if (cleandata[i][11]+cleandata[i][13])!=(cleandata[j][11]+cleandata[j][13]):
            distance+=10
        #Bkplane
        if cleandata[i][16]!=cleandata[j][16] or cleandata[i][17]!=cleandata[j][17] or cleandata[i][20]!=cleandata[j][20] or cleandata[i][21]!=cleandata[j][21]:
            distance+=1
        #VMFrac
        if cleandata[i][29]!=cleandata[j][29] or cleandata[i][34]!=cleandata[j][34]:
            distance+=1
...
def curve_prediction(xtest,clf):
    bone=[]
    for i in range (0,len(xtest)):
        if xtest[i][-1] not in bone:
            bone+=[xtest[i][-1]]
    ...
    for i in range(0,len(xtest)):
        for j in range(0,len(bone)):
            if xtest[i][-1]==bone[j][0]:
                bone[j]+=clf.predict([xtest[i][:-1]]).tolist()
...
if __name__ == "__main__":
    if len(sys.argv) >= 5:
        ...
        sys.exit('Usage: python Data_auto_process.py <xls file> <Element> <Split percentage> <Balance>')
    if boo=='T':
        balance=True
    else:
        balance=False
